...
(function () {
  ...
  function trackContactClick(kind, link) {
    var label = (link.innerText || link.textContent || '').replace(/\s+/g, ' ').trim() || kind;
    trackEvent(kind === 'phone' ? 'phone_click' : 'email_click', {
      contact_type: kind,
      link_text: label,
  ...
  function wireModals() {
    ...
    document.addEventListener('click', function (e) {
      var contactLink = e.target.closest('a[href]');
      if (contactLink) {
        var href = contactLink.getAttribute('href') || '';
        if (/^tel:/i.test(href)) trackContactClick('phone', contactLink);
        else if (/^mailto:/i.test(href)) trackContactClick('email', contactLink);
      }
    ...
    if (estimateForm) {
      estimateForm.addEventListener('submit', function (e) {
        ...
        trackEvent('generate_lead', {
          form_name: 'modal_estimate_form',
          lead_source: 'website_modal',
          project_type: formData.get('type') || 'unspecified',
          has_email: formData.get('email') ? 'yes' : 'no',
          page_path: window.location.pathname,
          page_title: document.title,
        });
